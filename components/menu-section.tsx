...
export function MenuSection() {
  return (
    <section id="carta" className="relative overflow-hidden bg-[#f4efe8] py-16 md:py-20 lg:py-24">
      <div className="absolute inset-x-0 top-0 h-16 bg-gradient-to-b from-[#efe7de] via-[#f4efe8]/86 to-transparent" />
      <div className="absolute inset-x-0 top-0 h-10 bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.34),transparent_78%)]" />
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mb-10 text-center md:mb-16">
          ...
          <h2 className="mt-4 font-serif text-4xl font-medium leading-none text-foreground md:text-5xl lg:text-6xl">
            La Carta
          </h2>
          <div className="mx-auto mt-6 max-w-4xl space-y-4 text-lg leading-relaxed text-muted-foreground">
            <p>
        ...
        <div className="md:hidden">
          <Accordion
            ...
            className="mx-auto max-w-3xl rounded-[1.5rem] border border-border/80 bg-card/92 shadow-[0_24px_50px_rgba(0,0,0,0.07)] backdrop-blur-sm"
          >
            {menuCategories.map((category) => (
              <AccordionItem key={category.id} value={category.id} className="border-border px-5">
                <AccordionTrigger className="relative items-center justify-center py-5 text-center hover:no-underline [&>svg]:absolute [&>svg]:right-0 [&>svg]:top-1/2 [&>svg]:size-5 [&>svg]:-translate-y-1/2">
                ...
                <AccordionContent className="pb-5">
                  ...
                  <div className="mt-5 space-y-3">
                    {category.items.map((item, index) => (
                      <article
                        key={item}
                        className="rounded-[1rem] border border-border/70 bg-background/86 px-4 py-3"
                      >
              ...
              <div className="rounded-[2rem] border border-border/80 bg-card/92 p-8 shadow-[0_28px_60px_rgba(0,0,0,0.08)] backdrop-blur-sm md:p-10">
                <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                  <div>
                    ...
                    <h3 className="mt-3 font-serif text-3xl font-medium text-foreground md:text-4xl">
                      {category.title}
                    </h3>
                ...
                <div className="mt-8 grid gap-4 md:grid-cols-2">
                  {category.items.map((item, index) => (
                    <article
                      key={item}
                      className="rounded-[1rem] border border-border/70 bg-background/86 px-4 py-4 transition-colors hover:border-secondary hover:bg-background"
                    >
